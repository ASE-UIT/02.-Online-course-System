  ...
  return (
    <div className="bg-white rounded-md">
      <div className="title w-1/3">
        <header
          className="bg-error-100 text-error-700 text-center leading-[48px] text-text/lg/semibold w-[240px] h-[48px] rounded-br-[4px] inline-block hover:shadow-md hover:cursor-pointer"
        >
          Nội dung khóa học
        </header>
      </div>
      ...
      <p className="text-text/md/regular">
        {`${lessonParts?.length} phần - ${lessonParts?.reduce(
          ...
          (() => {
            const totalMinutes = lessonParts.reduce(
              (acc, part) =>
                acc +
                part.lessons.reduce((lessonAcc, lesson) => lessonAcc + parseFloat(lesson?.duration), 0),
              0
              ...
              >
                <div className="content-name flex gap-[10px] w-2/3">
                  ...
                  <h3 className="text-text/md/medium">
                    {`Phần ${part?.partNo}: ${part?.partName}`}
                  </h3>
                </div>
                <span className="text-text/md/regular w-1/3 text-right">
                  {`${part?.lessons?.length} bài giảng`}
                </span>
              </div>
