...
const levels = [
  { id: 1, name: "Người mới", value: "easy" },
  { id: 2, name: "Trung cấp", value: "medium" },
  { id: 3, name: "Chuyên gia", value: "hard" },
];
const lengths = [
...
const Filter = ({ onFilterChange }) => {
  ...
  const handleCheckboxChange = (rating) => {
    setSelectedRatings((prev) => {
      ...
      // console.log("Updated Ratings:", newRatings); // Log updated ratings
      return newRatings;
    });
  };
    ...
    <div className="p-5 flex flex-col gap-[20px] rounded-[20px] shadow-lg">
          ...
          {levels.map((lv) => (
            <div key={lv.id} className="flex items-center gap-[10px]">
              <Checkbox
                ...
                onCheckedChange={() => handleLevelChange(lv.value)}
                id={`level-${lv.value}`}
                className="cursor-pointer rounded-[2px]"
              />
              <label htmlFor={`level-${lv.value}`} className="cursor-pointer text-text/sm/regular">
                {lv.name}
              </label>
              <p className="text-black-100 text-text/sm/regular">(10,000)</p>
            </div>
      ...
      <div className="flex justify-between">
        ...
        <div className="button flex relative">
          <Button variant="gray" className="flex items-center gap-2" onClick={toggleDropdown}>
        ...
        <div className="lengthList flex flex-col gap-[10px]">
          {lengths.map((length) => (
            <div key={length.id} className="flex items-center gap-[10px]">
              <Checkbox
                checked={selectedLengths.some((item) => item.min === length.min && item.max === length.max)}
